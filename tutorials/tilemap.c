...
void tile_draw(int i, float x, float y, float w, float h) {
    ...
    switch (i) {
        ...
        case 2:
            ...
            al_draw_filled_triangle(x + w * 0.5,
                                    y + h * 0.125,
                                    x + w * 0.125,
                                    y + h * 0.875,
                                    x + w * 0.875,
        ...
        case 3:
            al_draw_filled_rectangle(x, y, x + w, y + h, black);
            if (icon) al_draw_scaled_bitmap(icon, 0, 0, 48, 48, x, y, w, h, 0);
            break;
    }
}
    ...
    for (y = 0; y < 100; y++) {
        for (x = 0; x < 100; x++) {
            ...
            al_draw_scaled_bitmap(
                tiles, u, v, 64, 64, x * 32, y * 32, 32, 32, 0);
        }
    }
        ...
        if (redraw && al_is_event_queue_empty(queue)) {
            ...
            if (font) {
                al_draw_filled_rounded_rectangle(
                    4, 4, 100, 30, 8, 8, al_map_rgba(0, 0, 0, 200));
                al_draw_textf(font,
